...
TEST(PQTest, kSmallestPairs) {
  vector<vector<int>> a = {{1, 7, 11}, {1, 1, 2}, {1, 2}};
  vector<vector<int>> b = {{2, 4, 6}, {1, 2, 3}, {3}};
  vector<int> k = {3, 2, 3};
  vector<vector<vector<int>>> golden = {
      {{1, 6}, {1, 4}, {1, 2}}, {{1, 1}, {1, 1}}, {{2, 3}, {1, 3}}};
  ...
  for (int i = 0; i < a.size(); ++i) {
    ...
    ASSERT_EQ(result.size(), golden[i].size())
        << "result vector has wrong length";
    for (int j = 0; j < result.size(); ++j) {
      EXPECT_EQ(result[j], golden[i][j])
          << "Vectors x and y differ at index " << j;
    }
  }
}

TEST(PQTest, topKFrequent) {
  vector<vector<int>> a = {{1, 1, 1, 2, 2, 3}, {1}};
  vector<int> k = {2, 1};
  vector<vector<int>> golden = {{1, 2}, {1}};
  vector<int> result;
  ASSERT_EQ(a.size(), k.size());
  for (int i = 0; i < a.size(); ++i) {
